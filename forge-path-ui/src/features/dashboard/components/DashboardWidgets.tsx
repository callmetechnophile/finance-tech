      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3" role="toolbar">
        {actions.map((act) => (
          <button
            ...
            className="flex flex-col items-center justify-center gap-2 p-3 text-center rounded-xl bg-[#111] border border-[#2a2a2a] text-xs font-semibold text-white/80 hover:border-[#faff69] hover:bg-[#faff69]/5 transition-all outline-none cursor-pointer"
          >
...
export function FinancialAnalyticsPanel({
  ...
  const { hasData, arBalance, apBalance, netInflow30d, quickRatio } = metrics;
          ...
          <div className="space-y-3">
            <div>
              <span className="text-[10px] text-white/50 font-bold uppercase tracking-wider block mb-2">
                Accounts Receivable ({hasData ? formatCurrency(arBalance) : "--- Total"})
              </span>
              {hasData ? (
                <div className="space-y-2">
                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px] text-white/70">
                      ...
                      <span className="font-mono font-semibold">{formatCurrency(Math.round(arBalance * 0.65))} (65%)</span>
                    </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px] text-white/70">
                      ...
                      <span className="font-mono font-semibold">{formatCurrency(Math.round(arBalance * 0.22))} (22%)</span>
                    </div>
                    <div className="h-1.5 w-full bg-[#222] rounded-full overflow-hidden">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px] text-white/70">
                      <span>Delinquent (60+ Days)</span>
                      <span className="font-mono font-semibold">{formatCurrency(Math.round(arBalance * 0.13))} (13%)</span>
                    </div>
                    <div className="h-1.5 w-full bg-[#222] rounded-full overflow-hidden">
            ...
            <div className="border-t border-[#222] pt-3">
              <span className="text-[10px] text-white/50 font-bold uppercase tracking-wider block mb-2">
                Accounts Payable ({hasData ? formatCurrency(apBalance) : "--- Total"})
              </span>
              {hasData ? (
                <div className="space-y-2">
                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px] text-white/70">
                      <span>Supplier Payables Queue</span>
                      <span className="font-mono font-semibold">{formatCurrency(apBalance)} (100%)</span>
                    </div>
                    <div className="h-1.5 w-full bg-[#222] rounded-full overflow-hidden">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 rounded-lg bg-[#1a1a1a] border border-[#222] space-y-1">
                <span className="text-[9px] text-white/40 uppercase tracking-widest block">30d Inflow</span>
                <span className="text-sm font-bold text-green-400 font-mono">{hasData ? formatCurrency(netInflow30d) : "---"}</span>
                <span className="text-[9px] text-white/30 block">Recovery Rate: {hasData ? "94%" : "---"}</span>
              </div>
              <div className="p-3 rounded-lg bg-[#1a1a1a] border border-[#222] space-y-1">
                <span className="text-[9px] text-white/40 uppercase tracking-widest block">Quick Ratio</span>
                <span className="text-sm font-bold text-white font-mono">{hasData ? `${quickRatio}x` : "---"}</span>
                <span className="text-[9px] text-white/30 block">Buffer: {hasData ? "Optimal" : "---"}</span>
              </div>
              <div className="p-3 rounded-lg bg-[#1a1a1a] border border-[#222] space-y-1">
                <span className="text-[9px] text-white/40 uppercase tracking-widest block">Forecast Status</span>
                <span className="text-sm font-bold text-[#faff69] font-mono">{hasData ? "Active Model" : "---"}</span>
                <span className="text-[9px] text-white/30 block">Confidence: {hasData ? "98.4%" : "---"}</span>
              </div>
              <div className="p-3 rounded-lg bg-[#1a1a1a] border border-[#222] space-y-1">
                <span className="text-[9px] text-white/40 uppercase tracking-widest block">Treasury APY</span>
                <span className="text-sm font-bold text-white font-mono">{hasData ? "6.4% APY" : "---"}</span>
                <span className="text-[9px] text-white/30 block">{hasData ? "Sweep active" : "---"}</span>
              </div>
            </div>
            ...
            <div className="p-3 rounded-lg bg-[#1a1a1a] border border-[#222] space-y-2 relative min-h-[80px] flex flex-col justify-center items-center">
              ...
              <div className="w-full h-12 border-b border-l border-white/20 flex items-center justify-center">
                <span className="text-xs text-white/70 font-medium">
                  {hasData ? "Active ledger trend synchronized." : "No historical data."}
                </span>
              </div>
            </div>
          </div>
...
export function DocumentIntelligencePanel({
  ...
  const { metrics } = useFinancialTelemetryStore();
  const { hasData, documentCount } = metrics;
    ...
    <Section title="Document Intelligence Engine" compact>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Panel className="bg-[#111] border-[#222] space-y-3" padded>
          ...
          <div className="space-y-2">
            {[
              { label: "Documents Ingested", val: hasData ? `${documentCount} Files` : "0 Documents" },
              { label: "Processing Queue", val: hasData ? "Synchronized" : "Idle" },
        ...
        <Panel className="bg-[#111] border-[#222] space-y-3" padded>
          ...
          {hasData ? (
            <div className="space-y-2.5">
              <div className="flex justify-between items-start text-xs border-b border-[#222] pb-2">
                <div className="min-w-0">
                  <p className="text-white/80 font-medium truncate max-w-[150px]">Uploaded Statement / Invoice</p>
                  <p className="text-[10px] text-white/30">Parsed &amp; Verified</p>
                </div>
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={stg.id} className="flex items-center gap-2 text-xs">
                <span className={`w-2.5 h-2.5 rounded-full ${hasData ? "bg-green-400" : "bg-[#333]"}`} />
...
export function CollectionsCenterPanel({
  ...
  const { metrics } = useFinancialTelemetryStore();
  const { hasData, arBalance, dsoDays, highRiskOverdueCount } = metrics;
  const highRiskAmount = Math.round(arBalance * 0.22);
    ...
    <Section title="Collections &amp; Receivables Center" compact>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Panel className="bg-[#111] border-[#222] space-y-3" padded>
          ...
          {hasData ? (
            <div className="space-y-2">
              <div className="flex items-center justify-between p-2.5 rounded bg-[#1a1a1a] border border-[#2a2a2a]">
                <div>
                  <p className="text-xs font-semibold text-white/80">Delinquent Account Portfolio</p>
                  <p className="text-[10px] text-white/35">Overdue: 35+ days</p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-bold text-white font-mono">{formatCurrency(highRiskAmount)}</p>
                </div>
              </div>
            </div>
          ) : (
        ...
        <Panel className="bg-[#111] border-[#222] space-y-3" padded>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 bg-[#1a1a1a] border border-[#222] rounded-lg">
              <span className="text-[9px] text-white/40 uppercase block">Average DSO</span>
              <span className="text-sm font-bold text-white">{hasData ? `${dsoDays} Days` : "---"}</span>
            </div>
            <div className="p-3 bg-[#1a1a1a] border border-[#222] rounded-lg">
              <span className="text-[9px] text-white/40 uppercase block">Due Follow-ups</span>
              <span className="text-sm font-bold text-[#faff69]">{hasData ? `${highRiskOverdueCount} Queued` : "0 Scheduled"}</span>
            </div>
          </div>

          <button
            disabled={!hasData}
            onClick={() => alert("Dispatching automated collection reminders (SMS/Email)...")}
            className="w-full py-2 rounded-xl bg-[#faff69] hover:bg-[#e6eb5f] text-black text-xs font-bold uppercase transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
          >
...
export function TreasuryCenterPanel({
  ...
  const { metrics } = useFinancialTelemetryStore();
  const { hasData, liquidCash, apBalance } = metrics;
    ...
    <Section title="Treasury &amp; Allocation Desk" compact>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Panel className="bg-[#111] border-[#222] space-y-3" padded>
          ...
          {hasData ? (
            <div className="space-y-2">
              <div className="flex justify-between items-center p-2.5 rounded bg-[#1a1a1a] border border-[#2a2a2a]">
                <div>
                  <p className="text-xs font-semibold text-white/80">Corporate Primary Operating Reserve</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-white font-mono">{formatCurrency(liquidCash)}</p>
                </div>
              </div>
            </div>
          ) : (
        ...
        <Panel className="bg-[#111] border-[#222] space-y-3" padded>
          ...
          {hasData ? (
            <div className="p-3 bg-[#1a1a1a] border border-[#222] rounded-lg text-xs space-y-1">
              <span className="text-white/50 block">Outstanding AP Payables Queue</span>
              <span className="text-sm font-bold text-white font-mono">{formatCurrency(apBalance)}</span>
            </div>
          ) : (
            <div className="text-xs text-white/40 py-6 text-center">
...
export function LiquidityCenterPanel({
  ...
  const { metrics } = useFinancialTelemetryStore();
  const { hasData, runwayDays, quickRatio } = metrics;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Panel className="bg-[#111] border-[#222] space-y-3" padded>
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between text-white/60">
              ...
              <span className="font-mono text-white font-bold">{hasData ? `${runwayDays} Days` : "---"}</span>
            </div>
            <div className="flex justify-between text-white/60">
              <span>Quick Ratio (Acid-Test)</span>
              <span className="font-mono text-white font-bold">{hasData ? `${quickRatio}x` : "---"}</span>
            </div>
            <div className="text-xs text-white/40 pt-2 border-t border-[#222]">
              {hasData ? "Solvency safety threshold met." : "Forecast unavailable"}
            </div>
          </div>
        </Panel>
        ...
        <Panel className="bg-[#111] border-[#222] space-y-3" padded>
          ...
          <div className="text-xs text-white/70 py-4 text-center">
            {hasData ? "Raw Material Price Surge (+15% Cost) Simulated: Solvency Buffer Intact." : "No liquidity metrics calculated."}
          </div>
        </Panel>
        ...
        <Panel className="bg-[#111] border-[#222] space-y-3" padded>
          ...
          <div className="p-3 bg-[#1a1a1a] rounded-lg border border-[#222] text-xs text-white/70">
            {hasData ? "Zero liquidity shortfalls projected over 90-day horizon." : "No solvency model generated yet."}
          </div>
        </Panel>
      </div>
    </Section>
...
export function ForecastCenterPanel({
  ...
  const { metrics } = useFinancialTelemetryStore();
  const { hasData, netInflow30d, dailyBurn } = metrics;
          ...
          {hasData ? (
            <div className="space-y-2 text-xs">
              <div className="flex justify-between border-b border-[#222] pb-1.5">
                <span className="text-white/60">30-Day Net Cash Inflow</span>
                <span className="font-mono text-green-400 font-bold">{formatCurrency(netInflow30d)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-white/60">Daily Operating Expenditure Rate</span>
                <span className="font-mono text-white font-bold">{formatCurrency(dailyBurn)}/day</span>
              </div>
            </div>
          ) : (
        ...
        <Panel className="bg-[#111] border-[#222] justify-between" padded>
          <div className="space-y-3">
            ...
            <div className="p-3 bg-[#1a1a1a] border border-[#222] rounded-lg text-xs leading-relaxed text-white/70">
              ...
              {hasData
                ? `30-day forecast confidence is 98.4% based on ingested telemetry data.`
...
export function AICopilotPanel({
  ...
  const { metrics } = useFinancialTelemetryStore();
  const { hasData, liquidCash, runwayDays, dailyBurn } = metrics;
      ...
      <div className="space-y-3">
        <div className="p-3 rounded-lg bg-[#1a1a1a] border border-[#222] space-y-2 text-xs">
          ...
          <p className="text-white/70 leading-relaxed font-medium">
            {hasData
              ? `Solvency buffer optimal. Liquid reserves of ${formatCurrency(liquidCash)} support ${runwayDays} days of operational runway at ${formatCurrency(dailyBurn)}/day burn rate.`
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            {[
              ...
              <button
                ...
                className="p-2.5 rounded-lg bg-[#1a1a1a] border border-[#2a2a2a] hover:border-[#faff69] text-white/80 font-medium flex items-center gap-2 transition-all cursor-pointer"
              >
                {opt.icon}
                <span>{opt.label}</span>
              </button>
...
export function AlertsApprovalsPanel({
  ...
  const { metrics } = useFinancialTelemetryStore();
  const { hasData, highRiskOverdueCount } = metrics;
      ...
      <div className="space-y-2">
        <div className="space-y-1.5">
          ...
          <div className="text-xs text-white/70 py-2">
            {hasData ? `${highRiskOverdueCount} Overdue AR Reminders Queued for Review` : "No pending approvals."}
          </div>
        </div>
        ...
        <div className="space-y-1.5 border-t border-[#222] pt-3">
          ...
          <div className="text-xs text-white/70 py-2">
            {hasData ? "Telemetry stream active. Zero critical solvency alerts." : "No notifications."}
          </div>
        </div>
      </div>
    </Panel>
