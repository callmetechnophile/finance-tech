...
export function CustomerPortfolioRegion() {
  ...
  const { hasData, arBalance, activeAccounts, highRiskOverdueCount, dsoDays } = metrics;
  const highRiskAmount = Math.round(arBalance * 0.22);
  const recoveredAmount = Math.round(arBalance * 0.65);
    ...
    <Section title="Customer Portfolio &amp; Accounts Receivable Overview" compact>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3" aria-label="Customer portfolio metric indicators">
        <MetricCard
          ...
          value={hasData ? formatCurrency(arBalance) : "---"}
        ...
        <MetricCard
          ...
          value={hasData ? `${activeAccounts} Clients` : "0 Clients"}
          trend={{ value: hasData ? "Active" : "---", direction: "flat" }}
          severity="normal"
          icon={<Users className="w-4 h-4 text-white/40" aria-hidden="true" />}
        />
        <MetricCard
          label="High Risk Overdue"
          value={hasData ? formatCurrency(highRiskAmount) : "---"}
          trend={{ value: hasData ? `${highRiskOverdueCount} Accounts Escalated` : "---", direction: "flat" }}
          severity={hasData ? "critical" : "normal"}
          icon={<AlertTriangle className="w-4 h-4 text-red-400" aria-hidden="true" />}
        />
        <MetricCard
          label="Average DSO"
          value={hasData ? `${dsoDays} Days` : "---"}
        ...
        <MetricCard
          ...
          value={hasData ? formatCurrency(recoveredAmount) : "---"}
