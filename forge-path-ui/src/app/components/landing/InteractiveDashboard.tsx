...
export default function InteractiveDashboard() {
  ...
  return (
    <section ref={ref} className="relative min-h-screen bg-[#0b0e11] flex flex-col justify-center items-center overflow-hidden px-6 py-20 select-none border-t border-[#2b3139]">
      ...
      {/* Mini Mock Dashboard */}
      <div className="relative w-full max-w-4xl p-6 rounded-3xl bg-[#1e2329] border border-[#2b3139] shadow-2xl z-10 space-y-6">
        {/* KPI Grid */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Card 1 */}
          <div className="p-5 rounded-2xl bg-[#0b0e11] border border-[#2b3139] flex flex-col justify-between text-left">
            <span className="text-[10px] font-bold text-[#707a8a] uppercase tracking-wider">Current Cash Buffer</span>
            <div className="flex items-baseline gap-2 mt-2">
              <span className="text-2xl font-extrabold text-white font-mono">
                ${cash.toLocaleString()}
              </span>
              <span className="text-[10px] text-[#0ecb81] font-bold flex items-center gap-0.5">
                <ArrowUpRight className="w-3 h-3" /> +4.2%
              </span>
            </div>
            <span className="text-[9px] text-[#707a8a] font-semibold mt-1 uppercase">68 Days Runway</span>
          </div>

          {/* Card 2 */}
          <div className="p-5 rounded-2xl bg-[#0b0e11] border border-[#2b3139] flex flex-col justify-between text-left">
            <span className="text-[10px] font-bold text-[#707a8a] uppercase tracking-wider">Outstanding Receivables</span>
            <div className="flex items-baseline gap-2 mt-2">
              <span className="text-2xl font-extrabold text-white font-mono">
                ${receivables.toLocaleString()}
              </span>
              <span className="text-[10px] text-[#f6465d] font-bold flex items-center gap-0.5">
                <ArrowUpRight className="w-3 h-3" /> +12%
              </span>
            </div>
            <span className="text-[9px] text-[#707a8a] font-semibold mt-1 uppercase">12 Overdue Invoices</span>
          </div>

          {/* Card 3 */}
          <div className="p-5 rounded-2xl bg-[#0b0e11] border border-[#2b3139] flex flex-col justify-between text-left">
            <span className="text-[10px] font-bold text-[#707a8a] uppercase tracking-wider">Liquidity Score</span>
            <div className="flex items-baseline gap-2 mt-2">
            ...
            <span className="text-[9px] text-[#707a8a] font-semibold mt-1 uppercase">Burn Rate: $5k/day</span>
          </div>
        </div>

        {/* Chart Card */}
        <div className="p-6 rounded-2xl bg-[#0b0e11] border border-[#2b3139] text-left">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h4 className="text-xs font-bold text-white uppercase tracking-wider">Cash Runway Projection</h4>
              <p className="text-[9px] text-[#707a8a] mt-0.5 uppercase font-semibold">Actuals vs AI forecast</p>
            </div>
            <div className="flex items-center gap-1.5 text-xs text-[#fcd535] font-bold cursor-pointer">
          ...
          <div className="relative h-48 w-full overflow-hidden flex items-end">
            <svg className="w-full h-full" viewBox="0 0 800 200" fill="none" xmlns="http://www.w3.org/2000/svg">
              {/* Grid Lines */}
              <line x1="0" y1="50" x2="800" y2="50" stroke="#2b3139" strokeDasharray="3 3" />
              <line x1="0" y1="100" x2="800" y2="100" stroke="#2b3139" strokeDasharray="3 3" />
              <line x1="0" y1="150" x2="800" y2="150" stroke="#2b3139" strokeDasharray="3 3" />
