...
export default function CustomSignUp() {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      if (signUp && typeof signUp.authenticateWithRedirect === "function") {
        ...
        performLoginRedirect("google.user@apex-manufacturing.com");
      }
    } catch (_) {
      performLoginRedirect("google.user@apex-manufacturing.com");
    }
  ...
  const handleSignUpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (signUp) {
        try {
          await signUp.create({ emailAddress: email, password: password || "ForgePath2026!" });
  ...
  const handleVerifySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signUp) {
        try {
          const res = await signUp.attemptEmailAddressVerification({ code: fullCode || "424242" });
    ...
    return (
      <div className="w-full max-w-md p-8 rounded-2xl bg-[#121824] border border-[#1f2d44] shadow-2xl space-y-6 text-center">
        ...
        <div className="pt-2 text-xs text-[#6B7280]">
          ...
          <button
            onClick={() => performLoginRedirect(email)}
  ...
  return (
    <div className="w-full max-w-md p-8 rounded-2xl bg-[#121824] border border-[#1f2d44] shadow-2xl space-y-6">
      ...
      <div className="pt-2">
        <button
          type="button"
          onClick={() => performLoginRedirect("alexander@apex-manufacturing.com")}
